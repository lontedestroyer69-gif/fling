...
-- PERBAIKAN: Menambahkan fungsi getFunctionsInCategory yang hilang
...
FunctionManager:register("Sound GUI", function()
	...
	local function GSSVM_fake_script() 
		...
		parent.Activated:Connect(function()
			...
			if existingSound then
				...
				existingSound.Volume = tonumber(Volume) or 0.5
				existingSound.Looped = (looped == "True")
				existingSound:Play()
			else
				...
				clonedSound.Volume = tonumber(Volume) or 0.5
				clonedSound.Looped = (looped == "True")
				clonedSound:Play()
			end

			parent.Text = "Injected Sound Succesfully!"
	...
	local function MWUVEW_fake_script() 
	...
	local function BIKZQ_fake_script() 
	...
	local function AXYPXC_fake_script() 
		...
		if part:IsA("BasePart") and not part.Anchored and not part:IsDescendantOf(workspace.Terrain) then
			...
			if not isInCharacter and (part.Position - origin).Magnitude <= radius then
				if not CollectionService:HasTag(part, "OrbitPart") then
					pcall(function() part:SetNetworkOwner(speaker) end)
					part.CanCollide = false
...
-- =============================================================================
-- AUTOMATIC EXPLOIT SYSTEM FOR REMOTES (BUYDEVPRODUCT)
-- =============================================================================
FunctionManager:register("Exploit DevProduct", function()
	local remotesFolder = ReplicatedStorage:FindFirstChild("Remotes")
	if not remotesFolder then 
		Notify("Folder 'Remotes' tidak ditemukan!", "ERROR", 3)
		return 
	end
	
	local buyRemote = remotesFolder:FindFirstChild("BuyDevProduct")
	if not buyRemote then 
		Notify("Remote 'BuyDevProduct' tidak ditemukan!", "ERROR", 3)
		return 
	end

	-- GANTI "TARGET_ITEM_HERE" DENGAN TEKS STRING YANG DIDAPAT DARI SIMPLESPY
	local namaItemTarget = "TARGET_ITEM_HERE" 

	if namaItemTarget == "TARGET_ITEM_HERE" then
		Notify("Ganti 'TARGET_ITEM_HERE' di script terlebih dahulu!", "WARNING", 3)
		return
	end

	pcall(function()
		buyRemote:FireServer(namaItemTarget)
	end)
	Notify("Remote ditembak: " .. namaItemTarget, "SYSTEM", 3)
end, "Troll", "Uji kerentanan transaksi string bypass")

-- Update grid setelah semua terdaftar
updateGrid()
